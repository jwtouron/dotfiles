...
refresh="pkill -SIGRTMIN+10 i3blocks"
ds=@DEFAULT_SINK@

case "$1" in
    "inc") pactl set-sink-volume "$ds" "+${2}%"; $refresh ;;
    "dec") pactl set-sink-volume "$ds" "-${2}%"; $refresh ;;
    "mute") pactl set-sink-mute "$ds" toggle; $refresh ;;
esac
