...
-- Returns the path of program, or nil if cannot be found.
local function get_path(program)
    local path = os.getenv("PATH")
    for dir in string.gmatch(os.getenv("PATH"), "[^:]+") do
        local full_path = dir .. "/" .. program
        if gears.filesystem.file_executable(full_path) then
            return full_path
        end
    end
    return nil
...
-- Returns the program whose path is first found or nil, if none can be found.
local function find_one_of(programs)
    for i = 1, #programs do
        if get_path(programs[i]) then
            return programs[i]
        end
    end
    return nil
...
-- Global values
local terminal = os.getenv("TERMINAL") or find_one_of({"st", "xterm", "kitty", "termite"})
local browser = os.getenv("BROWSER") or find_one_of({"brave", "brave-browser", "firefox"})
...
mytextclock = wibox.widget.textclock('<span font="' .. wibar_font .. '">%a, %d %b %Y, %H:%M</span>')
...
awful.screen.connect_for_each_screen(function(s)
    ...
    s.mycpu = lain.widget.cpu {
        settings = function()
            ...
            widget:set_markup('<span font="' .. wibar_font .. '"> </span><span ' .. color .. ' font="' .. wibar_font .. '">' .. cpu_now.usage .. '%</span>')
        end
    }

    -- Create mem widget
    s.mymem = lain.widget.mem {
        settings = function()
            local color = ""
            ...
            widget:set_markup('<span font="' .. wibar_font .. '"> </span><span ' .. color .. ' font="' .. wibar_font .. '">' .. mem_now.perc .. '%</span>')
        end
    }
    ...
    s.mywibox:setup {
        ...
        { -- Right widgets
            ...
            make_separator(),
            volume_widget {widget_type = "icon_and_text", font = wibar_font},
            s.mycpu.widget,
            s.mymem.widget,
            mytextclock,
            logout_popup.widget{},
...
globalkeys = gears.table.join(
    ...
    -- Prompt
    awful.key({ modkey },            "r",     function () awful.spawn.with_shell("fzfmenu_run") end,
